...
│   ├── pane.py                    # 서브에이전트 패인 상태 추적 (태스크 목표/설명, waiting_approval ⏸)
│   ├── tui.py                     # curses 기반 split-pane TUI (좌: 태스크 목표, 우: Claude Code 스타일 도구 출력)
...
테일 스택:
 ...
 12. HumanInTheLoopMiddleware    — HITL 인터럽트 (메인 에이전트)

서브에이전트 전용 스택 (SubagentHITLMiddleware):
 13. SubagentHITLMiddleware      — IPC 큐 기반 서브에이전트 HITL (layers/subagent_hitl.py)
```
...
#### 서브에이전트 HITL (SubagentHITLMiddleware)

서브에이전트는 별도 프로세스에서 실행되므로 LangGraph의 `interrupt()` 메커니즘을 사용할 수 없다.
대신 `SubagentHITLMiddleware` (`totoro/layers/subagent_hitl.py`)가 IPC 큐를 통해 HITL을 처리한다:

```
서브에이전트 프로세스에서 위험한 도구 호출 감지
    │
    ▼
[1] SubagentHITLMiddleware.after_model()에서 interrupt_on 매칭 확인
[2] allow_patterns 매칭 시 자동 승인 (바이패스)
[3] event_queue에 hitl_request 전송 (SubagentEvent)
[4] response_queue에서 블로킹 대기 (timeout: 300초)
    │
    ▼ (부모 프로세스 - TUI 측)
[5] TUI가 curses를 일시 해제하고 승인 프롬프트 표시
[6] 사용자 선택: (a)pprove / (A)pprove all / (r)eject / (e)dit
[7] 대기 중인 모든 요청을 일괄 처리 (batch processing)
[8] response_queue에 결정 전송, TUI curses 복원
    │
    ▼
[9] 서브에이전트가 결정에 따라 도구 실행/거부/수정
```

**"Approve All"은 글로벌** — 한 번 선택하면 `_auto_approve` 플래그가 설정되어 해당 세션의
모든 서브에이전트에서 이후 도구 실행을 자동 승인한다. `_runtime_auto_approve` 모듈 레벨 플래그가
`_run_parallel()` 호출 간에 상태를 유지하며, 새 TUI 인스턴스에도 전파된다 (검증→수정→재검증 루프 지원).

**권한 패턴** (`.totoro/settings.json`):

```json
{"permissions": {"allow": ["mkdir", "npm *", "write_file", "*.py", "*"]}}
```

`_matches_allow()` 함수가 도구 이름, execute 명령어 첫 단어, 파일 경로 glob을 매칭한다.
...
### 5.3 서브에이전트 HITL (IPC 큐 방식)

서브에이전트는 별도 `multiprocessing` 프로세스에서 실행되므로 LangGraph의 `interrupt()`를
직접 사용할 수 없다. 대신 `SubagentHITLMiddleware`가 IPC 큐를 통해 HITL을 처리한다:
...
SubagentHITLMiddleware
  .after_model() 호출
    ...
    │
    ├── approve → 도구 실행
    ├── approve_all → _auto_approve=True, 이후 전부 자동
    ├── reject → ToolMessage(status="error")
    └── edit → 수정된 args로 도구 실행
```

### 5.4 Tatsuo 검증 순서 및 자동 재시도

tatsuo(리뷰어)는 worker 에이전트(satsuki, mei, susuwatari 등)가 모두 완료된 후에 순차 실행된다.
병렬 실행이 아닌 순차 실행으로 변경하여, worker의 결과물을 검증할 수 있게 한다.
...
| 타임아웃 | 값 | 설명 |
|----------|-----|------|
| 첫 이벤트 (메인) | 3분 | 첫 스트리밍 이벤트가 오지 않으면 타임아웃 |
| 첫 이벤트 (서브에이전트) | 5분 | 서브에이전트는 더 긴 초기화 시간 허용 |
| 절대 타임아웃 | 10분/서브에이전트 | 서브에이전트당 최대 실행 시간 |

에러 분류: rate limit → 대기 후 재시도, auth → 키 재확인, network → 재연결, timeout → 중단.
