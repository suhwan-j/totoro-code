...
class SubagentEvent:
    ...
    label: str
    # "ai_text"|"tool_start"|"tool_end"|"diff"|"done"|"error"
    event_type: str
...
class ToolCall:
    ...
    result_preview: str = ""
    file_path: str = ""
    content_lines: list = field(
        default_factory=list
    )  # preview lines for write/edit
...
class PaneState:
    ...
    _pending_tool_queue: list = field(
        default_factory=list
    )  # queue of (name, args) from tool_start events
...
class PaneManager:
    ...
    def add_subagent(
        self, label: str, description: str, pid: int | None = None
    ):
        ...
        """
        with self._lock:
            self.panes[label] = PaneState(
                label=label, description=description, pid=pid
            )
    ...
    def update_subagent(self, event: SubagentEvent):
        ...
        with self._lock:
            ...
            if event.event_type == "ai_text":
                ...
                summary = event.data.get(
                    "summary", event.data.get("name", "?")
                )
    ...
    def get_summary(self) -> str:
        ...
        with self._lock:
            ...
            for pane in self.panes.values():
                ...
                line = (
                    f"  {icon} {_BOLD}{pane.label}"
                    f"{_RESET} ({pane.elapsed},"
                    f" {pane.tool_count} tools"
                    f"{files_str}{tok_str})"
                )
                parts.append(line)
    ...
    def is_active(self) -> bool:
        ...
        with self._lock:
            return any(
                p.status in ("running", "waiting_approval")
